...
async function handleSaveCurrentPage() {
  try {
    const savedPage = await PageStorage.saveCurrentPage(document.title, window.location.href);
...
function createSavedPagesPanel() {
  ...
  header.querySelector('#knotery-close-panel')?.addEventListener('click', () => {
    isOpen = false;
    panel.style.display = 'none';
  });

  header.querySelector('#knotery-export-json')?.addEventListener('click', async () => {
    try {
...
async function loadSavedPages() {
  ...
  try {
    content.innerHTML = '<div style="text-align: center; color: #666;">読み込み中...</div>';
    
    const pages = await PageStorage.getAllPages();
    
    if (pages.length === 0) {
      content.innerHTML = '<div style="text-align: center; color: #666;">保存されたページがありません</div>';
      return;
    }
    ...
    content.innerHTML = '<div style="text-align: center; color: #f44336;">読み込みに失敗しました</div>';
  }
}
